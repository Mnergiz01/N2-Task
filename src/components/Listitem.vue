<template>
  <div>
    <ul class="space-y-5 font-medium text-white">
      <!-- Kullanıcılar sayfası için menü -->
      <template v-if="route.path === '/users'">
        <li
          v-for="item in Menuitem1"
          :key="item"
          class="cursor-pointer flex items-center p-3 rounded-lg transition"
          :class="{
        ...
        >
          <svg
            class="w-5 h-5 me-3"
            :class="{ 'text-gray-800': isActive(item) }"
            fill="currentColor"
            viewBox="0 0 20 18"
          >
            <path
...
const handleMenuitem2 = async (item) => {
  const endpoint = item.toLowerCase()

  const userId = route.params.id

  const url = `https://jsonplaceholder.typicode.com/${endpoint}?userId=${userId}`

  try {
    const response = await axios.get(url)
    console.log(`${item} verileri:`, response.data)

    // Eğer veri geldiyse yönlendir
    if (response.data.length > 0) {
      router.push(`/${endpoint}/${userId}`)
    } else {
      console.warn(`${item} verisi bulunamadı.`)
    }
  } catch (error) {
    console.error(`${item} verileri alınırken hata oluştu:`, error)
  }
}
