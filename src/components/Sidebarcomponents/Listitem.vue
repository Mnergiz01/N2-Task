<template>
  <div>
    <ul class="space-y-5 font-medium text-gray-800">
      ...
      <template v-if="route.path === '/users'">
        <li v-for="item in Menuitem1" :key="item" class="cursor-pointer flex items-center p-3 rounded-lg transition"
          :class="{
            'bg-white text-purple-900 border-l-8 border-purple-500 shadow font-semibold': isActive(item),
            'hover:bg-purple-300 text-gray-800': !isActive(item)
          }" @click="handleMenuitem(item)">
          <svg class="w-5 h-5 me-3" :class="{ 'text-purple-900': isActive(item), 'text-gray-500': !isActive(item) }"
